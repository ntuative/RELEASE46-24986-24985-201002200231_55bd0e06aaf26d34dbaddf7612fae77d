   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            {
               Core.crash("Error catched in EventDispatcher; some event listener crashed!",var_1295.errorID,var_1295);
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_269[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1113) : uint(const_494);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_269)
            {
               _loc2_ = var_269[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_269[_loc1_];
            }
            var_269 = null;
